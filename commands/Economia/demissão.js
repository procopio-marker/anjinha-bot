...
exports.run = async (client, message, args) => {
  ...
  let saldo = await db.get(`money_${message.author.id}`) // puxando o 'dinheiro' armazenado na database
  
  //essa função, custará 3500 para ser efetuada!
  if (saldo < 3500) return message.reply('para pedir demissão, você necessita de **R$ 500**!') // caso o usuário não possua, daremos o erro
...
if (emprego === null) { // caso o 'emprego' seja null, ou seja, sem, iremos avisar que ele precisa ter um emprego
  ...
  if (emprego === 1) { // No arquivo para escolher um trabalho, definimos que 1 é igual à um Programador... Então, vamos fazer uma mensagem bonita
    message.channel.send(`Olá **${message.author.username}**, você realmente se demitir do cargo de Programador? Você pagara uma multa de **500 AngelsCoins**`).then(msg => {
    ...
    msg.awaitReactions(filter, {max: 1}) // retornando com a reação que puxamos acima
      ...
      .then(collected => { // mais uma função 'then', nomeada de 'collected' para definirmos que foi coletado
        ...
        if (reaction.emoji.name === '✅') { // caso o usuário clique no emoji referente à correto, iremos fazer a ação
          ...
          db.subtract(`dinheiro_${message.author.id}`, 500) // removendo do 'dinheiro' da database
  ...
  if (emprego === 2) { // agora, o emprego dois, que é o Designer
    message.channel.send(`Olá **${message.author.username}**, você realmente se demitir do cargo de Minerador? Você pagara uma multa de **500 AngelsCoins**`).then(msg => {
    ...
    msg.awaitReactions(filter, {max: 1}) // retornando com as reações
      .then(collected => { // e mais uma função then, que vai ser como o collected acima
        ...
        if (reaction.emoji.name === '✅') { // caso o usuário clique no emoji referente à correto, iremos fazer a ação
          ...
          db.subtract(`money_${message.author.id}`, 500) // removendo do 'dinheiro' na database
...
if (emprego === 3) { // agora, o emprego dois, que é o Designer
    message.channel.send(`Olá **${message.author.username}**, você realmente se demitir do cargo de Discord Staff? Você pagara uma multa de ** 500 AngelsCoins**`).then(msg => {
...
if (emprego === 4) { // agora, o emprego dois, que é o Designer
    message.channel.send(`Olá **${message.author.username}**, você realmente se demitir do cargo de Assasino? Você pagara uma multa de **500 AngelsCoins**`).then(msg => {
